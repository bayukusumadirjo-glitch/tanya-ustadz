...
LINK_DEPLOY = "https://tanya-ustadz-dirj.streamlit.app"   # UBAH SETELAH DAPAT LINK

# Auto refresh setiap berapa detik
...
# ===================================
# AUTO REFRESH COUNTDOWN DI SIDEBAR
# ===================================
if st.session_state.logged_in:
    placeholder = st.sidebar.empty()
    with placeholder.container():
        st.sidebar.markdown(f"**Auto Refresh:** {AUTO_REFRESH_SECONDS} detik")
        countdown = st.sidebar.empty()
        for i in range(AUTO_REFRESH_SECONDS, 0, -1):
            countdown.info(f"Refresh dalam {i} detik...")
            time.sleep(1)
        st.rerun()  # Ini yang bikin auto refresh

# ===================================
# LOGIN (Tahan Refresh!)
# ===================================
if not st.session_state.logged_in:
    st.title("KajianQNA – Login")
...
# ===================================
        ...
        for row in c.fetchall():
            ...
            if row[4] == 1:
                ...
                if cols[3].button("Aktifkan", key=f"aktif_{row[0]}"):
                    ...
                    st.rerun()
            if cols[4].button("Hapus", key=f"hapus_{row[0]}", type="secondary"):
    ...
    with tab2:
        st.subheader("Moderasi Pertanyaan (Auto Refresh)")
        c.execute("SELECT id FROM kajian WHERE aktif = 1")
        aktif_id = c.fetchone()
        if not aktif_id:
            st.info("Belum ada kajian aktif")
        else:
            c.execute("SELECT id, nama_penanya, isi, tanggal FROM pertanyaan WHERE kajian_id = ? AND approved = 0 ORDER BY tanggal DESC", (aktif_id[0],))
            waiting = c.fetchall()
            if not waiting:
    ...
    with tab3:
        ...
        col2.code(qr_link)
        st.markdown("Cetak & tempel di masjid → otomatis ikut kajian aktif!")
