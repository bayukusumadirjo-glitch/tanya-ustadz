...
# Tambah kolom nama_ustadz dan tanggal_kajian
...
if is_penanya:
    ...
    kajian_nama, ustadz, tgl_kajian = aktif
    st.success(f"**{kajian_nama}**")
        ...
        if kirim:
            if not nama.strip() or not pertanyaan.strip():
                ...
                c.execute("""INSERT INTO pertanyaan (kajian_id, nama_penanya, pertanyaan, tanggal, approved) 
                             VALUES (?, ?, ?, ?, 0)""", (kajian_id, nama.strip(), pertanyaan.strip(), tgl))
                conn.commit()
...
# Tampilkan info kajian aktif di atas (jika ada)
c.execute("SELECT nama, nama_ustadz, tanggal_kajian FROM kajian WHERE aktif = 1")
...
if role == "Ustadz":
    ...
    with tab1:
        ...
        if col1.button("Buat Kajian Baru", type="primary"):
            if nama_kajian:
    ...
    with tab2:
        if aktif_info:
            ...
            st.caption(f"Ustadz: {aktif_info[1]} • {aktif_info[2]}")
            c.execute("SELECT id FROM kajian WHERE aktif = 1")
            kajian_id = c.fetchone()[0]
    ...
    with tab3:
        ...
        LINK_KAMU = "https://tanya-ustadz-dirj.streamlit.app"  # UBAH INI SETELAH DEPLOY
        qr_link = f"{LINK_KAMU}?penanya=yes"
        qr_url = f"https://api.qrserver.com/v1/create-qr-code/?size=500x500&data={
